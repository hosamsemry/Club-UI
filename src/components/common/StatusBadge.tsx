...
const CONFIG: Record<string, { label: string; className: string; dot?: string }> = {
  ...
  pending: { label: 'Pending', className: 'bg-amber-50 text-amber-700 border-amber-200/60', dot: 'bg-amber-400' },
  confirmed: { label: 'Confirmed', className: 'bg-blue-50 text-blue-700 border-blue-200/60', dot: 'bg-blue-500' },
  cancelled: { label: 'Cancelled', className: 'bg-red-50 text-red-600 border-red-200/60' },
  ...
  partial: { label: 'Partial', className: 'bg-amber-50 text-amber-700 border-amber-200/60', dot: 'bg-amber-400' },
  unpaid: { label: 'Unpaid', className: 'bg-slate-50 text-slate-600 border-slate-200/60' },
  ...
  voided: { label: 'Voided', className: 'bg-gray-50 text-gray-400 border-gray-200/60 line-through' },
  // Boolean-like
  active: { label: 'Active', className: 'bg-green-50 text-green-700 border-green-200/60', dot: 'bg-green-500' },
  inactive: { label: 'Inactive', className: 'bg-gray-50 text-gray-400 border-gray-200/60' },
  open: { label: 'Open', className: 'bg-blue-50 text-blue-700 border-blue-200/60', dot: 'bg-blue-500' },
  closed: { label: 'Closed', className: 'bg-gray-50 text-gray-400 border-gray-200/60' },
};

export function StatusBadge({ status }: StatusBadgeProps) {
  const config = CONFIG[status] ?? { label: status, className: 'bg-gray-50 text-gray-600 border-gray-200/60' };
  return (
    <Badge variant="outline" className={`font-medium text-xs gap-1.5 px-2 py-0.5 ${config.className}`}>
      {config.dot && (
        <span className={`inline-block w-1.5 h-1.5 rounded-full ${config.dot} animate-pulse shrink-0`} />
      )}
      {config.label}
    </Badge>
  );
